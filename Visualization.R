getwd()
setwd("E:/Desktop/Project Work/dv")
library(shiny)
...
cn <- read.xlsx2("Country-Code.xlsx", sheetIndex = 1, header = TRUE, stringsAsFactors = FALSE)
...
ui <- dashboardPage(dashboardHeader(title="Zomato"),
                    dashboardSidebar(
                      sidebarMenu(menuItem("Location of All Restaurants",tabName = "map"), menuItem("Aggregate Rating", tabName = "dash1"), menuItem("Price Range", tabName = "dash2"), menuItem("Excellent Restaurants", tabName = "exe"),  
                                  
                                  menuItem("Analysis of India", menuSubItem("Restaurant Ratings", tabName = "mapind"), menuSubItem("Average Cost for 2", tabName = "avg_c"), menuSubItem("Price Range", tabName = "price_ind"), menuSubItem("Has Table Booking", tabName = "tb"), menuSubItem("Has Online Booking", tabName = "ob"), menuSubItem("Text Data Analysis of Most Popular Cuisines", tabName = "cuis")),
                    ...
                    dashboardBody(
                      tabItems(
                        ...
                        tabItem(tabName = "map", fluidRow(box(plotOutput("mapp")), box(plotOutput("locbar")))),
                        
                        tabItem(tabName = "dash1", fluidRow(box(plotOutput("histogram")), box(plotOutput("bar"))), fluidRow(box(sliderInput("bins","Number of breaks",1,100,50)))),
                        
                        tabItem(tabName = "dash2", fluidRow(box(plotOutput("line")), box(plotOutput("bars")), box(plotOutput("boxp")))),
...
server <- function(input, output) {
  ...
  plot(newmap, xlim = range(zomato$Longitude),ylim = range(zomato$Latitude), asp = 1, main="Distribution of All the Restaurants")
  ...
  output$histogram <- renderPlot({hist(zomato$Aggregate.rating, main="Aggregate Rating", breaks = input$bins)})
  
  
  output$bar <- renderPlot({plot(zomato$Aggregate.rating, col="blue", main="Aggregate Rating", xlab="Aggregate Rating of restaurants", cex=.4)})
  
  
  output$line <- renderPlot({plot(zomato$Restaurant.ID,zomato$Price.range,col="red", main="Price Range", ylab="Price Range of Restaurants")})
  ...
  barplot(price_tab, main = "Price Range of the Restaurants", ylab="No of Restaurants", xlab="Price Range")})
  
  
  output$exew <- renderPlot({zomatoE <- filter(zomato,zomato$Rating.text == "Excellent")
  newmap <- getMap(resolution = "low")
  plot(newmap, xlim = range(zomatoE$Longitude),ylim = range(zomatoE$Latitude), asp = 1,
  ...
  output$text<-renderText("\t\t\t\n\n\n Data visualisation Project by group : --> \n\t  Viraj Gupta : 19BCE2151 \n\t  ,	 Aditya Chaudhary : 19BCE0544 \n\t  ,	 Arpit Sharma : 19BDS0027")
